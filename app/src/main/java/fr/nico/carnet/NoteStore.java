...
public final class NoteStore extends SQLiteOpenHelper {
    ...
    public NoteStore(Context context) { super(context, "carnet.db", null, 2); }
    ...
    @Override public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion != 1 || newVersion != 2) throw new IllegalStateException("Migration requise");
    ...
    public int importBackup(JSONObject backup) throws JSONException {
        ...
        try {
            for (int i = 0; i < notes.length(); i++) {
                ...
                ContentValues values = content(body); long updated = checkedTime(n, "updated"); values.put("updated", updated);
